...
let user_access = localStorage.getItem('access')
  ? JSON.parse(localStorage.getItem('access'))[0]?.is_admin ?? false
  : false;
...
menuItems = user_access
  ? menuItems
  : menuItems.filter((menu) => menu['title'] !== 'Administration');
...
console.log('Inside Topmenu',topMenu);
export { topMenu };
