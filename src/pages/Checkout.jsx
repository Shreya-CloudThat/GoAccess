...
function Checkout() {
  ...
  useEffect(() => {
    // Fetch Media Items
    fetchData(
      'https://8v901o1eo2.execute-api.ap-south-1.amazonaws.com/dev/list-venues/listSectionsAndPricesbyPerformanceId',
      {
  ...
  // const tickets = bookingdetails?.map((item) => item.tickets?.data) ?? [];
  // console.log('bbokkkkk', tickets);

  let user_detail = JSON.parse(localStorage.getItem('access'))[0];
  console.log('user_detail', user_detail);
  console.log(bookingdetails[0]?.tickets?.data);
  return (
      ...
      <hr className="mt-2" />
      {
        !checkoutsStatus ? (
          <div class="w-full flex mt-5 gap-10 md:flex-wrap ">
            {/* ----------------------------------------- */}
            {/* <div class=" bg-gray-300 rounded-lg p-10 w-1/4 ">
          <p className="text-2xl font-bold">Ticket Details</p>
          <hr className="text-violet-800 bg-violet-400" />

          <div className=" mt-3 flex gap-3">
            <p className="inline font-base text-base mt-1">Available Seats : </p>
          </div>
        </div> */}
            ...
            <div class=" bg-gray-300 rounded-lg p-10 w-1/3">
              ...
              {/* {numberofTickets.adult.length > 0 && ( */}
              <button
                className="py-2 px-5 border border-black bg-violet-500 rounded-lg text-white hover:bg-violet-400 shadow-md shadow-black mt-4"
                onClick={checkout}
                disabled={numberofTickets.adult.length === 0 || !showTable}
              >
                Checkout
              </button>
              {/* )} */}
        ...
        //     </h2>
        //     <p className="text-green-500 text-lg">Your Booking is done!</p>
        //     <button
        //       className="mt-4 bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600"
        //       onClick={() => (window.location.href = '/about')}
        //     >
